...
<head>
    ...
    <title>BUD.ME - Pilih Lapangan</title>
    <style>
        body { 
        ...
        .lapangan-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-top: 20px; }
        .card { background: white; padding: 25px; border-radius: 15px; box-shadow: 0 5px 15px rgba(0,0,0,0.05); display: flex; flex-direction: column; justify-content: space-between; border: 1px solid #eee; border-top: 6px solid #4575e5; transition: 0.3s; }
        .card:hover { transform: translateY(-5px); box-shadow: 0 10px 25px rgba(69, 117, 229, 0.15); }
        .card h3 { margin: 0 0 15px 0; color: #1541a6; border-bottom: 1px dashed #ddd; padding-bottom: 10px; font-size: 18px; }
        .card p { margin: 8px 0; color: #555; font-size: 14px; }
        .price-tag { font-weight: 800; color: #4575e5; font-size: 16px; }
        
        .btn-pilih { display: block; text-align: center; background-color: #1541a6; color: white; padding: 12px; text-decoration: none; border-radius: 12px; font-weight: bold; margin-top: 20px; transition: 0.3s; }
        .btn-pilih:hover { background-color: #4575e5; }
...
<body>
    ...
    <div class="booking-banner">
        <h2>Pilih Jenis Lapangan</h2>
        <p>Pilih lapangan yang sesuai dengan preferensimu</p>
    </div>

    <div class="container">
        <div class="breadcrumb">Beranda > Pilih Cabang > <strong>Pilih Jenis Lapangan</strong></div>
        <h2>Pilih Jenis Lapangan</h2>
            ...
            <?php foreach($data_lapangan as $lap): ?>
                <div class="card">
                    <div>
                        ...
                        <p><strong>Jenis Lantai:</strong> <?= htmlspecialchars($lap['nama_jenis']); ?></p>
                        <p><strong>Harga Weekday:</strong> <span class="price-tag">Rp <?= number_format($lap['harga_weekday'], 0, ',', '.'); ?>/jam</span></p>
                        <p><strong>Harga Weekend:</strong> <span class="price-tag">Rp <?= number_format($lap['harga_weekend'], 0, ',', '.'); ?>/jam</span></p>
                    </div>
                    <a href="<?= BASE_URL ?>index.php?page=booking&step=3&lapangan_id=<?= $lap['lapangan_id'] ?>" class="btn-pilih">PILIH LAPANGAN</a>
                </div>
            <?php endforeach; ?>
        </div>
        <a href="<?= BASE_URL ?>index.php?page=booking&step=1" class="btn-kembali">&larr; Kembali Pilih Cabang</a>
    </div>
